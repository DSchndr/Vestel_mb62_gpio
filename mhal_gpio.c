...
#define MEM_SIZE 0x2065f8 //seems to be enough
...
void MHal_GPIO_Init(void)
{
	...
	printf("fd open success, (/dev/mem) fd: %d\n", fd);
	gpio_addr = mmap(0, MEM_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd, REG_MIPS_BASE);
        if(gpio_addr == MAP_FAILED) {
                printf("Unable to map IOMEM\n");
		return;
        }
	printf("mmap virtual addr:%p | size: %#x\n", gpio_addr, MEM_SIZE);
//seems like this is not needed?
	volatile unsigned int *addr = gpio_addr + MHal_Get_Reg_Address(REG_ALL_PAD_IN);;
	*addr  &= ~BIT7;
}

void MHal_GPIO_WriteRegBit(U32 u32Reg, U8 u8Enable, U8 u8BitMsk)
{
	volatile unsigned int *addr = gpio_addr + MHal_Get_Reg_Address(u32Reg);
   	if(u8Enable)
        	*addr |= u8BitMsk;
   	else
        	*addr &= (~u8BitMsk);
...
{
	volatile unsigned int *addr_r_oen = gpio_addr + MHal_Get_Reg_Address(gpio_table[u8IndexGPIO].r_oen);
    	*addr_r_oen &= (~gpio_table[u8IndexGPIO].m_oen);
}
...
{
	volatile unsigned int *addr_r_in = gpio_addr + MHal_Get_Reg_Address(gpio_table[u8IndexGPIO].r_in);
    	return ((*addr_r_in&gpio_table[u8IndexGPIO].m_in)? 1 : 0);
}

U8 MHal_GPIO_Pad_InOut(U8 u8IndexGPIO)
{
        volatile unsigned int *addr_r_oen = gpio_addr + MHal_Get_Reg_Address(gpio_table[u8IndexGPIO].r_oen);
    	return ((*addr_r_oen&gpio_table[u8IndexGPIO].m_oen)? 1 : 0);
}

void MHal_GPIO_Pull_High(U8 u8IndexGPIO)
{
        volatile unsigned int *addr_r_out = gpio_addr + MHal_Get_Reg_Address(gpio_table[u8IndexGPIO].r_out);
        *addr_r_out |= gpio_table[u8IndexGPIO].m_out;
}
...
{
        volatile unsigned int *addr_r_oen;
	addr_r_oen = gpio_addr + MHal_Get_Reg_Address(gpio_table[u8IndexGPIO].r_oen);
        *addr_r_oen &= (~gpio_table[u8IndexGPIO].m_oen);
        volatile unsigned int *addr_r_out;
	addr_r_out  = gpio_addr + MHal_Get_Reg_Address(gpio_table[u8IndexGPIO].r_out);
        *addr_r_out |= gpio_table[u8IndexGPIO].m_out;
}
...
void MHal_print() {
	U8 gpio_table_size = 206;
	U8 i = 0;
	for(i = 0; i<gpio_table_size; i++) {
		printf("OEN Register | I: %d  | addr: %#x | realaddr: %#x \n", i, gpio_table[i].r_oen, MHal_GPIO_REG(gpio_table[i].r_oen));
	}
        for(i = 0; i<gpio_table_size; i++) {
                printf("OUT Register | I: %d  | addr: %#x | realaddr: %#x \n", i, gpio_table[i].r_out, MHal_GPIO_REG(gpio_table[i].r_out));
        }
        for(i = 0; i<gpio_table_size; i++) {
                printf("IN Register | I: %d  | addr: %#x | realaddr: %#x \n", i, gpio_table[i].r_in, MHal_GPIO_REG(gpio_table[i].r_in));
        }
}


void MHal_GPIO_Dump() { //Dump gpio states
	printf("-->MHal_GPIO_Dump()\n");
        int i = 0;
	U8 u8IndexGPIO = 0;
       	for(i = 0; i<=206; i++) {
		u8IndexGPIO = i;
        	volatile unsigned int *addr_r_oen = gpio_addr + MHal_Get_Reg_Address(gpio_table[u8IndexGPIO].r_oen);
        	int oen = ((*addr_r_oen&gpio_table[u8IndexGPIO].m_oen)? 1 : 0);
        	volatile unsigned int *addr_r_out = gpio_addr + MHal_Get_Reg_Address(gpio_table[u8IndexGPIO].r_out);
        	int out = ((*addr_r_out&gpio_table[u8IndexGPIO].m_out)? 1 : 0);
        	volatile unsigned int *addr_r_in = gpio_addr + MHal_Get_Reg_Address(gpio_table[u8IndexGPIO].r_in);
        	int in  = ((*addr_r_in&gpio_table[u8IndexGPIO].m_in)? 1 : 0);
                printf("GPIO: %03d | IN: %d | OUT: %d | OEN: %d \n", i, in, out, oen);
        }
       	printf("\n");
}
